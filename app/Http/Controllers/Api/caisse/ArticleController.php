...
use App\Http\Resources\Api\caisse\ArticleCardResource;
use App\Http\Resources\Api\caisse\ArticleResource;
...
class ArticleController extends Controller
{
    ...
    public function recherche_liste_type(Request $request)
    {
        $mot = $request->get('search');
        $type = $request->get('type', 'designation');

        $query = Article::query();

        switch ($type) {
            case 'reference':
                $query->where('reference', $mot);
                break;
            case 'designation':
                $query->where('designation', 'like', '%' . $mot . '%');
                break;
            case 'code_barre':
                $query->where('code_barre', $mot);
                break;
        }
