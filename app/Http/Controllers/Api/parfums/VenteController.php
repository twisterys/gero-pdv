...
class VenteController extends Controller
{
    public function sauvegarder(Request $request)
    {
        ...
        try {
            ...
            $vente_tva = 0;
            $vente_reduction = 0;
            //Todo : Put reduction Here
            $vente_reduction_input = floatval($request->get('total_red')) ?? 0;
            if (count($lignes) > 0) {
                foreach ($lignes as $key => $ligne) {
                    $reduction_mnt = round($ligne['prix'],2) * ($vente_reduction_input / 100);
                    ...
                    $o_ligne->mode_reduction = 'pourcentage';
                    $o_ligne->nom_article = $ligne['name'];
                    $o_ligne->ht = $ligne['prix'];
                    $o_ligne->quantite = $ligne['quantity'];
                    $o_ligne->taxe = $o_article->taxe;
                    $o_ligne->reduction = $reduction_mnt;
                    $o_ligne->total_ttc = $this->calculate_ttc($o_ligne->ht, $o_ligne->reduction, $o_ligne->taxe, $o_ligne->quantite);
    ...
    public function sauvegarder_avec_paiement(Request $request)
    {
        ...
        try {
            ...
            if (count($lignes) > 0) {
                foreach ($lignes as $ligne) {
                    $ht = $ligne['prix'] ?? 0;
                    $reduction = round($ht * (($request->get('total_red') ?? 0) / 100), 2);
                    $taxe = $ligne['taxe'] ?? (Article::find($ligne['id'])->taxe ?? 0);
                    $quantite = $ligne['quantity'] ?? 0;
                    $htReduit = $ht - $reduction;
                    $ttc = round(($htReduit * (1 + $taxe / 100)) * $quantite, 2);
            ...
            if (count($lignes) > 0) {
                foreach ($lignes as $key => $ligne) {
                    ...
                    $o_ligne->quantite = $ligne['quantity'];
                    $o_ligne->taxe = $o_article->taxe;
                    $o_ligne->reduction = $vente_reduction_input ?? 0;
                    $o_ligne->mode_reduction = 'pourcentage';
                    $o_ligne->total_ttc = $this->calculate_ttc($o_ligne->ht, $reduction_mnt, $o_ligne->taxe, $o_ligne->quantite);
                    $o_ligne->position = $key;
                    $o_ligne->magasin_id = $o_pos_session->magasin_id;
                    $o_ligne->save();
                    // ------------------ ### Stock ### ------------------
                    ...
                    $vente_ht += ($o_ligne->ht) * $o_ligne->quantite;
                    $vente_reduction += $reduction_mnt;
                    $vente_tva += $this->calculate_tva_amount($o_ligne->ht, $reduction_mnt, $o_ligne->taxe, $o_ligne->quantite);
                    $vente_ttc += $o_ligne->total_ttc;
                }
                $o_vente->update([
                    'total_ht' => $vente_ht,
