...
use App\Http\Resources\Api\commercial\HistoryResource;
...
class VenteController extends Controller
{
    public function sauvegarder(Request $request)
    {
        $rules = [
            ...
            'methode_livraison'=>'nullable|exists:methode_livraisons,id'
        ];
        $attributes = [
            'lignes' => 'lignes',
            'lignes.*.id' => 'article',
        ...
        try {
            ...
            $data = [
                ...
                'note' => null,
                'commercial_id'=>$request->get('commercial')??null,
                'methode_livraison_id'=>$request->get('methode_livraison')?? null,
            ];
            ...
            if (count($lignes) > 0) {
                foreach ($lignes as $key => $ligne) {
                    ...
                    $o_ligne->total_ttc = $this->calculate_ttc($o_ligne->ht, $reduction, $o_ligne->taxe, $o_ligne->quantite);
    ...
    public function sauvegarder_avec_paiement(Request $request)
    {
        ...
        $rules = [
            ...
            'commercial'=>'nullable|exists:commercials,id',
            'methode_livraison'=>'nullable|exists:methode_livraisons,id'
        ];
        $attributes = [
            'lignes' => 'lignes',
        ...
        try {
            ...
            if (count($lignes) > 0) {
                foreach ($lignes as $key => $ligne) {
                    ...
                    $o_ligne->quantite = $ligne['quantity'];
                    $o_ligne->taxe = $o_article->taxe;
                    $o_ligne->reduction = $request->get('reduction') ?? 0;
                    $o_ligne->total_ttc = $this->calculate_ttc($o_ligne->ht, $reduction, $o_ligne->taxe, $o_ligne->quantite);
                    $o_ligne->position = $key;
                    $o_ligne->magasin_id = $o_pos_session->magasin_id;
                    $o_ligne->save();
