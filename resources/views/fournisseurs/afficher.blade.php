...
@section('document-title', ucwords($o_fournisseur->nom))
@push('styles')
                        ...
                        <div id="__fixed" class="d-flex switch-filter justify-content-between align-items-center">
                            <div>
                                ...
                                <h5 class="m-0 float-end ms-3"><i class="mdi mdi-contacts me-2 text-success"></i>
                                    {{ucwords($o_fournisseur->nom)}} <span class="text-muted font-size-10">({{$o_fournisseur->reference}})</span>
                                </h5>
                            </div>
                            <div class="pull-right">
                                <a href="{{route('fournisseurs.modifier',$o_fournisseur->id)}}"
                                   class="btn btn-soft-warning"><i
                    ...
                    <div class="row mx-0">
                        <div
                            ...
                            <div class="rounded-circle overflow-hidden border border-white border-5  bg-white"
                                ...
                                <img src="https://placehold.co/150x150?text={{$o_fournisseur->reference}}"
                                     class="border-0 w-100" alt="">
                            </div>
                            <h5 class="mb-0 mt-2 text-white text-center">{{$o_fournisseur->nom}} </h5>
                            <p class="text-center text-white-50  mb-0">{{$o_fournisseur->reference}}
                                - {{strtoupper($o_fournisseur->forme_juridique->nom)}}</p>
                        </div>
                        <div class=" p-3 row col-xxl-10 col-lg-9 col-md-8 col-12 align-items-start">
                            <div class="col-12 row">
                                ...
                                <div class="col-xxl-3  col-sm-6 col-12  my-1 my-xxl-0  d-flex align-items-center">
                                    ...
                                    <div class="ms-3 ">
                                        <span class="font-weight-bolder font-size-sm">Total décaissement</span>
                                        <p class="mb-0 h5 text-black">{{number_format($decaissement,3,'.','')}} MAD</p>
                                    </div>
                                </div>
                                <div class="col-xxl-3  col-sm-6 col-12  my-1 my-xxl-0  d-flex align-items-center">
                                    <div
                                    ...
                                    <div class="ms-3 ">
                                        ...
                                        <p class="mb-0 h5 text-black">{{number_format($credit,3,'.','')}} MAD</p>
                                    </div>
                                </div>
                                <div class="col-12 ">
                                    <hr class="border">
                            ...
                            <div class="col-lg-6 row m-0">
                                <p class=" col-xxl-12 col-lg-3 col-sm-6"><i
                                        class="fa fa-envelope me-2"></i> {{$o_fournisseur->email??'-'}}</p>
                                <p class="text-capitalize  col-xxl-12 col-lg-3 col-sm-6"><i
                                        class="fa fa-phone fa-flip-horizontal me-2"></i> {{$o_fournisseur->telephone ?? '-'}}
                                </p>
                                <p class="text-capitalize col-xxl-12 col-lg-3 col-sm-6"><i
                                        class="fa fa-id-badge me-2"></i> {{$o_fournisseur->ice??'-'}}</p>
                                <p class="text-capitalize  col-xxl-12 col-lg-3 col-sm-6"><i
                                        class="fa fa-location-arrow me-2"></i> {{$o_fournisseur->adresse??'-'}}</p>
                            </div>
                            <div class="col-lg-6  m-0">
                                <p class=" " > <b>Limite de crédit :</b> {{$o_fournisseur->limite_de_credit??'-'}} MAD</p>
                                <p class=" " > <b>RIB :</b> {{$o_fournisseur->rib??'-'}}</p>
                                <p class="text-capitalize " > <b>Note:</b> <br> {{$o_fournisseur->note??'-'}}</p>
                            </div>
            ...
            <div class="card">
                <div class="card-body">
                            ...
                            <a class="nav-link active p-3" data-bs-toggle="tab" href="#ventes-impaye"
                               role="tab">
                                <div class="d-flex align-items-center">
                                    ...
                                    <h5 class="m-0">Achats impayés ({{$o_fournisseur->achatsImpaye()->count()}})</h5>
                                </div>
                            </a>
                        </li>
                        @foreach($types as $type)
                            <li class="nav-item">
                                <a class="nav-link ventes-tabs  p-3" data-type="{{$type}}" data-paiement="{{in_array($type,$payables) ? 1 : 0}}" data-bs-toggle="tab" href="#ventes-{{$type}}"
                                   role="tab">
                                    <div class="d-flex align-items-center">
                                        <i class="fa fa-chart-line text-success me-3"></i>
                                        <h5 class="m-0"> @lang('achats.'.$type.'s') ({{$o_fournisseur->achats()->whereRaw('Year(date_emission) = '.session()->get('exercice'))->where('type_document',$type)->count()}})</h5>
                                    </div>
                                </a>
                            </li>
                        @endforeach
                    ...
                    <div class="tab-content">
                        <input type="hidden" id="fournisseur-input" value="{{$o_fournisseur->id}}">
                        <div class="tab-pane active p-3" id="ventes-impaye" role="tabpanel">
                            <table class="table table-striped rounded overflow-hidden">
                                ...
                                @forelse($o_fournisseur->achatsImpaye as $achat)
                                    <tr>
                                        <td>{{$achat->reference}}</td>
                                        <td>{{$achat->date_emission}}</td>
                                        <td>{{$achat->total_ttc}} MAD</td>
                                        <td>{{$achat->credit?? '0.00'}} MAD</td>
                                        <td>{{$achat->debit}} MAD</td>
                                        <td><a class="btn btn-sm btn-soft-primary"
                                               ...
                                               href="{{route('achats.afficher',[$achat->type_document,$achat->id])}}"><i
...
@push('scripts')
    ...
    <script>
        const __dataTable_filter_inputs_id = {
            fournisseur_id: '#fournisseur-input',
        }
        ...
        $(document).on('click','.ventes-tabs',function (){
            ...
            let url = "{{url('achats/')}}/"+nav_tab.data('type')+'/liste';
