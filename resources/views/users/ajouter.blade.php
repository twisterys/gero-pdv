...
@section('document-title','Ajouter un utilisateur')
@push('styles')
...
@section('page')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('utilisateurs.sauvegarder')}}" method="post">
                        @csrf
                        <!-- #####--Card Title--##### -->
                            ...
                            <div class="col-md-4 col-12 mt-2">
                                ...
                                <select name="i_role" id="roles" class="form-select  @error('i_role') is-invalid @enderror">
                                   @foreach($roles as $role)
                                        <option @selected(old('i_role') == $role->name) value="{{$role->name}}">{{$role->name}}</option>
                                   @endforeach
                                </select>
                                @error('i_role')
                                <div class="invalid-feedback">
                            ...
                            <div class="col-md-4 col-12 mt-2">
                                ...
                                <select name="i_dashboard" id="dashboard" class="form-select @error('i_dashboard') is-invalid @enderror">
                                    @foreach($dashboards as $dashboard)
                                        <option
                                            @selected(in_array($dashboard->id, (array) old('i_dashboard', [])))
                                            value="{{ $dashboard->id }}">
                                            {{ $dashboard->name }}
                                        </option>
                                    @endforeach
                                    ...
                                    <div class="col-md-4 col-12 mt-2">
                                        ...
                                        <select name="i_magasins[]" id="magasins" class="form-select @error('i_magasins') is-invalid @enderror" multiple>
                                            @foreach($magasins as $magasin)
                                                <option @selected(in_array($magasin->id, old('i_magasins', []))) value="{{$magasin->id}}">{{$magasin->reference}}</option>
                                            @endforeach
                                        </select>
                                        @error('i_magasins')
                                        <div class="invalid-feedback">
