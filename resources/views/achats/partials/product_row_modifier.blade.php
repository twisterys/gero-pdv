...
<tr>
    <td>
        <input type="hidden" class="article_id" name="lignes[{{$position}}][i_article_id]" value="{{$o_ligne['i_article_id']?? ($o_ligne['article_id'] ?? null)}}">
        <input type="hidden" class="article_reference" name="lignes[{{$position}}][i_article_reference]" value="{{$o_ligne['i_article_reference']?? ($o_ligne['article']['reference'] ?? null)}}">
        <div class="input-group mb-2">
                @if($o_ligne['i_article_id'] ?? ($o_ligne['article_id']?? false))
                    <span class="input-group-text" >{{$o_ligne['i_article_reference']?? ($o_ligne['article']['reference'] ?? null)}}</span>
                @endif
    ...
    <td>
        ...
        @enderror
        <select class="form-select row_select2 {{$errors->has('lignes.'.$position.'.i_unite')? 'is-invalid' : ''}}" style="width: 120px" name="lignes[{{$position}}][i_unite]" id="">
    ...
    <td>
        ...
        <input style="width: 120px" class="form-control prix_ht {{$errors->has('lignes.'.$position.'.i_prix_ht')? 'is-invalid' : ''}}"  type="number"
               name="lignes[{{$position}}][i_prix_ht]" value="{{number_format($o_ligne['i_prix_ht']?? ($o_ligne['ht'] ?? null),2,'.','')}}">
