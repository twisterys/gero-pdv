        ...
        @foreach(($ac['clients'] ?? []) as $cl)
            ...
            <tr>
                ...
                @foreach(($ac['articles'] ?? []) as $art)
                    ...
                    <td class="center">
                        @if($q != 0 || $ttc != 0)
                            {{ format_decimal(round_number($q)) }}
                        @endif
                    </td>
                @endforeach
                <td class="right">{{ format_decimal(round_number($totClient['total_ttc'] ?? 0)) }}</td>
                <td class="right">{{ format_decimal(round_number($totClient['total_paye'] ?? 0)) }}</td>
                <td class="right">{{ format_decimal(round_number($creance)) }}</td>
            </tr>
        @endforeach
        <tr>
            ...
            @foreach(($ac['articles'] ?? []) as $art)
                <td></td>
            @endforeach
            <td class="right"><strong>{{ format_decimal(round_number($ac['totals']['total_ttc'] ?? 0)) }}</strong></td>
            <td class="right"><strong>{{ format_decimal(round_number($ac['totals']['total_paye'] ?? 0)) }}</strong></td>
            <td class="right"><strong>{{ format_decimal(round_number($ac['totals']['total_creance'] ?? 0)) }}</strong></td>
        </tr>
        </tbody>
    </table>
@else
    ...
    <table>
        ...
        @foreach($cr['rows'] as $row)
            <tr>
                ...
                <td class="right">{{ format_decimal(round_number($row['total_paiement_today'] ?? 0)) }}</td>
                <td class="right">{{ format_decimal(round_number($row['total_ttc'] ?? 0)) }}</td>
                <td class="right">{{ format_decimal(round_number($row['creance_amount'] ?? 0)) }}</td>
            </tr>
        @endforeach
        <tr>
            <td colspan="9"><strong>Total des paiements</strong></td>
            <td colspan="2" class="right"><strong>{{ format_decimal(round_number($cr['total_paiements'] ?? 0)) }}</strong></td>
        </tr>
        </tbody>
    </table>
@else
...
@if(!empty($depenses))
    <table>
        ...
        @forelse(($depenses['items'] ?? []) as $row)
            <tr>
                <td>{{ $row['categorie'] }}</td>
                <td class="right">{{ format_decimal(round_number($row['montant'] ?? 0)) }}</td>
            </tr>
        @empty
            <tr>
        ...
        <tr>
            <td><strong>Total</strong></td>
            <td class="right"><strong>{{ format_decimal(round_number($depenses['total'] ?? 0)) }}</strong></td>
        </tr>
        </tbody>
    </table>
@else
...
<table>
    ...
    <tr>
        <td>Total ventes</td>
        <td colspan="2"></td>
        <td class="right">{{ format_decimal(round_number($tr['total_vente_jour'] ?? 0)) }}</td>
    </tr>
    <tr>
        <td>Espèces</td>
        <td class="right">{{ format_decimal(round_number($tr['total_espece_jour'] ?? 0)) }}</td>
        <td class="right">{{ format_decimal(round_number($tr['total_espece_creance'] ?? 0)) }}</td>
        <td class="right">{{ format_decimal(round_number($tr['total_espece'] ?? 0)) }}</td>
    </tr>
    <tr>
        <td>Chèques</td>
        <td class="right">{{ format_decimal(round_number($tr['total_cheque_jour'] ?? 0)) }}</td>
        <td class="right">{{ format_decimal(round_number($tr['total_cheque_creance'] ?? 0)) }}</td>
        <td class="right">{{ format_decimal(round_number($tr['total_cheque'] ?? 0)) }}</td>
    </tr>
    <tr>
        <td>LCN</td>
        <td class="right">{{ format_decimal(round_number($tr['total_lcn_jour'] ?? 0)) }}</td>
        <td class="right">{{ format_decimal(round_number($tr['total_lcn_creance'] ?? 0)) }}</td>
        <td class="right">{{ format_decimal(round_number($tr['total_lcn'] ?? 0)) }}</td>
    </tr>
    <tr>
        <td>Dépenses</td>
        <td colspan="2"></td>
        <td class="right">{{ format_decimal(round_number($tr['total_depenses'] ?? 0)) }}</td>
    </tr>
    <tr>
        <td><strong>Reste en caisse</strong></td>
        <td colspan="2"></td>
        <td class="right"><strong>{{ format_decimal(round_number($tr['reste_en_caisse'] ?? 0)) }}</strong></td>
    </tr>
    </tbody>
</table>
