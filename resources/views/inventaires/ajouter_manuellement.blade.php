...
@endpush
@section('page')
    <form action="{{ route("inventaire-importer-stocks-manuellement") }}" method="POST" class="needs-validation" novalidate
          autocomplete="off">
        @csrf
        <div class="row">
            <div class="col-12">
                            ...
                            <div class="row col-md-12 ">
                                ...
                                <div class="col-12 col-lg-3 col-md-4 mb-3">
                                    ...
                                    <select class="form-select " id="type" name="type">
                                        <option value="ordinaire" {{ old('type', 'ordinaire') == 'ordinaire' ? 'selected' : '' }}>Ordinaire</option>
                                        <option value="extra_ordinaire" {{ old('type') == 'extra_ordinaire' ? 'selected' : '' }}>Extra Ordinaire</option>
                                    </select>
                                </div>
                            ...
                            <div data-simplebar="init" class="table-responsive col-12 mt-3">
                                <table class="table rounded overflow-hidden table-hover table-striped" id="table">
                                    ...
                                    <tr class="bg-primary text-white ">
                                        ...
                                        <th class="text-white" style="width: 20%;white-space: nowrap;">Quantité actuelle</th>
                                        <th class="text-white" style="width: 20%;white-space: nowrap;">Nouvelle quantité</th>
                                    ...
                                    @if (old('lignes'))
                                        @foreach (old('lignes') as $key => $ligne)
                                            @include('inventaires.partials.product_row_ajouter')
                                        @endforeach
                                    @else
                                        @include('inventaires.partials.product_row_ajouter')
                                    @endif
...
@push('scripts')
    ...
    <script>
        const __row = `@include('inventaires.partials.product_blank_row')`;
        const __articles_modal_route = "{{ route('articles.article_select_modal', ['type' => 'inventaire']) }}";
    </script>
