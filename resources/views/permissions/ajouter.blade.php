...
@section('page')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form id="articles-form" enctype="multipart/form-data" action="{{route('permissions.sauvegarder')}}"
                                        ...
                                        <div class="card shadow-sm h-100">
                                            <div class="card-header">
                                                <div
                                                    ...
                                                    <div class="d-flex align-items-center">
                                                        <input name="{{$card}}" value="1" type="checkbox" class="wild-card" id="{{$card}}"
                                                               switch="bool">
                                            ...
                                            <div class="card-body ">
                                                @foreach($permissions->filter(function ($item) use($card) {
                                                                    return false !== stristr($item->name, $card) && !str_contains($item->name,'.*') ;
                                                                                                    }) as $permission)
                                                    <div class="d-flex justify-content-between align-items-center">
                                                        <label for="{{$permission->name}}" class="h4">@lang('permissions.'.$permission->name)</label>
