    ...
    <tr>
        <td style="width: 50%;padding-right: 1rem;" >
            <p class="reference-container">
                @lang('achats.'.$type) : {{$o_achat->reference ?? 'Brouillon'}}
            </p>
            <p style="padding: .5rem 0; text-align: center" >@lang('achats.'.$type.'.date_emission'):{{$o_achat->date_emission}}/ @if(in_array($type,['dv','fa','fp','bc']))
                    @lang('achats.'.$type.'.date_expiration')
                    {{$o_achat->date_expiration}}
                @endif</p>
        </td>
        <td style="width: 50%; padding-left: 1rem" >
            <div class="info-container">
                <p>Fournisseur: {{$o_achat->fournisseur->nom}}</p>
                <p>{{$o_achat->fournisseur->adresse}}</p>
                <p>ICE :{{$o_achat->fournisseur->ice}}</p>
                <p>Tél : {{$o_achat->fournisseur->telephone}}</p>
            </div>
        </td>
    </tr>
</table>
<div class="content" style="padding-top: 2rem">
    ...
    <div class="col-md-12 "
        ...
        <table id="table-articles" style="height: {{$table_height}}px;max-height: {{$table_height}}px"
            ...
            <tbody id="items">
            @foreach($o_achat->lignes as $index => $ligne)
                @php
            ...
            @endforeach
            <tr class="note">
                <td colspan="6" class="note" style="height: {{160 + ($o_achat->note ? 70 : 0)}}px;">
                </td>
            </tr>
            </tbody>
        </table>
    ...
    <div class="col-md-12 total"
         style="page-break-inside: avoid; position: fixed; bottom: {{$o_template->image_en_bas_hauteur-100-($o_achat->note ? 0 : 150)}}px; height: {{310 + ($o_achat->note ? 70 : 60)}}px;width: 100%">
        <div class="total-container " style="page-break-inside: avoid">
            ...
            @if( $o_achat['total_reduction'] != 0)
                <div class="total-line">
                    <h5>Réduction</h5> <h5>{{$o_achat->total_reduction}} MAD</h5>
                </div><hr>

            @endif
            <div class="total-line">
                <h5>Total TVA</h5> <h5>{{$o_achat->total_tva}} MAD</h5>
            </div><hr>
            <div class="total-line">
                <h5 style="color: white">Total TTC</h5>
                <h2 style="color: #fff;">{{$o_achat->total_ttc}} MAD</h2>
            </div>
        </div>
    ...
    <table class="signature" style="page-break-inside: avoid; position: fixed; bottom: {{$o_template->image_en_bas_hauteur-100-($o_achat->note ? 0 : 150)}}px; height: {{160 + ($o_achat->note ? 70 : 80)}}px;width: 100%">
        <tr>
            <th><u>Signature de fournisseur</u></th>
