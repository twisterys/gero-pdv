...
<div class="content " style="padding-top: 2rem;width:100%;">
    <div class="">
        <div class="to_address">
            ...
            <table style="width: 100%; padding-left:0px;">
                <tr>
                    <td style="white-space: nowrap; vertical-align: top">Client:</td>
                ...
                <tr>
                    ...
                    <td style="text-align: right"> {{$o_vente->client->adresse}}</td>
                </tr>
            </table>
        </div>
        <div class="side-left">
            <div><h1 style="margin: 0">@lang('ventes.'.$type)</h1></div>
            <table style="width: 50%">
                <tr>
                    <td style="width: 100%">Référence:</td>
                    <td style="text-align:right;">{{$o_vente->reference ?? 'Brouillon'}}</td>
                </tr>
                <tr>
                    <td>@lang('ventes.'.$type.'.date_emission')</td>
                    <td style="text-align:right;">{{$o_vente->date_emission}}</td>
                </tr>
                @if(in_array($type,['dv','fa','fp','bc']))
                    <tr>
                        <td>@lang('ventes.'.$type.'.date_expiration')</td>
                        <td style="text-align:right;">{{$o_vente->date_expiration}}</td>
                    </tr>
                @endif
            </table>
        </div>
        ...
        <table id="table-articles" style="height: {{$table_height}}px;max-height: {{$table_height}}px;"
            ...
            @foreach($o_vente->lignes as $ligne)
                ...
                <tr class="items" style="width:100%; margin:0px;">
                    ...
                    <td style='vertical-align: top;text-align: center;white-space: nowrap'>{{number_format($ligne->ht,3,'.',' ')}} </td>



                    @if( $o_vente['total_reduction'] != 0)
                    <td style='vertical-align: top;text-align: center;white-space: nowrap'>{{number_format($ligne->reduction,3,'.',' ')}} {{$ligne->mode_reduction === 'fixe' ? ' ':'%'}}</td>
                    @endif
                    <td style='vertical-align: top;text-align: center;'>{{$ligne->taxe}} %</td>
                    <td style="padding-right:1.2cm;vertical-align: top;white-space: nowrap">{{number_format(  (($ligne->ht-$reduction) *$ligne->quantite) + (($ligne->ht-$reduction) *$ligne->quantite * $ligne->taxe/100)  ,3,'.',' ')}} </td>
                </tr>
            @endforeach
            <tr>
    ...
    <div style="clear: both"></div>
    <div class="col-md-12 "
         style="page-break-inside: avoid; position: fixed; bottom: {{$o_template->image_en_bas_hauteur-100-($o_vente->note ? 0 : 40)}}px; height: {{310 + ($o_vente->note ? 70 : 0)}}px;width: 100%">
        <div class="total-container " style="page-break-inside: avoid;">
            ...
            @if( $o_vente['total_reduction'] != 0)
                <div class="total-line">
                <h5>Réduction</h5> <h5>{{number_format($o_vente->total_reduction,3,'.',' ')}} MAD</h5>
            </div><hr>
    ...
    <table class="signature" style="page-break-inside: avoid; position: fixed; bottom: {{$o_template->image_en_bas_hauteur-100-($o_vente->note ? 0 : 40)}}px; height: {{160 + ($o_vente->note ? 70 : 0)}}px;width: 100%">
        <tr>
