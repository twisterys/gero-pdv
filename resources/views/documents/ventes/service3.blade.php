...
<div class="content " style="padding-top: 2rem;width:100%;">
        ...
        <table id="table-articles" style="height: {{$table_height}}px;max-height: {{$table_height}}px;"
            ...
            @foreach($o_vente->lignes as $ligne)
                ...
                <tr class="items" style="width:100%; margin:0px;">
                    ...
                    <td style='vertical-align: top;text-align: center;white-space: nowrap'>{{number_format($ligne->ht,2,'.',' ')}} </td>



                    @if( $o_vente['total_reduction'] != 0)
                    <td style='vertical-align: top;text-align: center;white-space: nowrap'>{{number_format($ligne->reduction,2,'.',' ')}} {{$ligne->mode_reduction === 'fixe' ? ' ':'%'}}</td>
                    @endif
                    <td style='vertical-align: top;text-align: center;'>{{$ligne->taxe}} %</td>
                    <td style="padding-right:1.2cm;vertical-align: top;white-space: nowrap">{{number_format(  (($ligne->ht-$reduction) *$ligne->quantite) + (($ligne->ht-$reduction) *$ligne->quantite * $ligne->taxe/100)  ,2,'.',' ')}} </td>
                </tr>
            @endforeach
            <tr>
    ...
    <div class="col-md-12 "
        ...
        <div class="total-container " style="page-break-inside: avoid;">
            ...
            @if( $o_vente['total_reduction'] != 0)
                <div class="total-line">
                <h5>Réduction</h5> <h5>{{number_format($o_vente->total_reduction,2,'.',' ')}} MAD</h5>
            </div><hr>
